...
const countNumberOfElements = (arr) => {
  let total=arr.reduce((acc,item,ind)=>{
 return ind+1
  },0)
  return total;
  // Solution code here...
...
const returnNames = (arr) => {
  let namearray=arr.reduce((acc,item)=>{
  acc.push(item.name);
  return acc;
  },[])
  return namearray
...
const reversedString = (str) => {
  ...
  console.log(arrStr)
  let revirseString =arrStr.reduce((acc,item)=>{
   acc=item+acc;
   return acc;
  },'')
  return revirseString
  // Solution code here...
};
