...
const screenForNames = (arr) => {
  let reg=/^(Mr.|Mrs.|Ms.|Dr.)(\s)(\w{1,})/g;

  let newarray =arr.filter(item=>{
    console.log(item,reg.test(item) ,item.match(reg))
  return(item,reg.test(item) ,item.match(reg))
  })
  return(newarray)
...
let biggerThanLuke = (arr) => {
  let str=""
    let luke=arr.find(item=>{
      return(item.name.includes("Luke"))
    }) 
   let names=arr.filter(item=>{
  
     if (parseInt(item.mass)>parseInt (luke.mass)){
       return(true)
     }else{
       return false
     }
