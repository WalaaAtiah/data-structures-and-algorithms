...
const isCapitalized = (str) => {
  let req1=/\b[A-Z](\w)*/g;
  return (req1.test(str)?str.match(req1):[]) 
  // Solution code here...
};
...
for (let i=0;i<arr.length;i++){
  newarra.push(arr[i].replace(wordSeparators,'')) ;
}
  console.log(newarra)
  let string =newarra.join(' ')
  console.log(string)
  let req=/\b[A-J](\w)*/g;
 return (req.test(string)?string.match(req):[])
  // Solution code here...
};
/* ------------------------------------------------------------------------------------------------
