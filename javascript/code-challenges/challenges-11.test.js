...
function transformToLis(obj) {
  let array = [];
  for (let prop in obj) {
    array.push(`<li>${prop}: ${obj[prop]}</li>`);
  }
  return array;
  // Solution code here...
}
...
const divisibleByFiveTwoToThePower = (input) => {
  let result = input.map((item) => {
    let arr = [];
    let newarray = item.filter((j) => {
      if (typeof j == "number" && j % 5 == 0) {
...
let findShortest = (data) => {
  let name = "";
  let tollest = data.reduce((acc, item) => {
    if (Number(item.height) < Number(acc.height)) {
      acc = item;
      name = item.name;
    }
    return acc;
